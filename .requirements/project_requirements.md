...
- **FR-I-06** — Eine Rechnung enthält **eine oder mehrere** Positionszeilen, je eine pro abgerechnetem Monat. Mid-Month-Start/-End wird je Zeile als Pro-rata berechnet. Einzel-Monats-Rechnungen sind der häufige Spezialfall mit genau einer Positionszeile.
...
- **FR-I-09** — **Konflikt-Modus „Mit selber Rechnungsnummer neu erstellen"**: Wenn für einen Monat bereits Rechnungen existieren, kann der Nutzer wählen, alle editierbaren Rechnungen zu löschen und unter ihrer **ursprünglichen Nummer** neu zu erstellen. Geschützte Rechnungen (Versendet / Bezahlt) bleiben unverändert und werden im UI als „übersprungen" angezeigt. Der bestehende Modus „Alle überschreiben" verwendet weiterhin neue Nummern, lässt geschützte Rechnungen aber ebenfalls unangetastet. Für **mehrmonatige** Rechnungen (FR-I-10) ist dieser Modus nicht verfügbar — der Coach muss die bestehende Rechnung manuell löschen und neu anlegen.
- **FR-I-10** — **Mehrmonatige Einzelrechnung**: Im Einzelrechnungs-Flow wählt der Coach pro Rechnung beliebig viele Monate als Positionszeilen aus (Zeilen-Builder mit „+ weiteren Monat hinzufügen"). Jeder ausgewählte Monat wird zu einer eigenen Positionszeile mit eigenem Pro-rata-Zeitraum, eigener Beschreibung („Coaching {Monat} {Jahr}") und eigenem Betrag. Eine Rechnung verbraucht genau **eine** Rechnungsnummer unabhängig von der Zeilenanzahl. Duplikat-Monate innerhalb derselben Rechnung werden beim Absenden blockiert. Der Monatsbatch (FR-I-01) bleibt einmonatig pro Athlet, überspringt jedoch Athleten, deren Zielmonat bereits durch eine bestehende (einmalige oder mehrmonatige) Rechnung abgedeckt ist.
- **FR-I-11** — **Bereits-abgerechnet-Hinweise und Athleten-Übersicht**: Der Einzelrechnungs-Flow zeigt für den ausgewählten Athleten oberhalb der Zeilen eine Zusammenfassung der bereits abgerechneten Monate sowie je Zeile einen Inline-Hinweis, falls der gewählte Monat schon abgerechnet ist („⚠ bereits abgerechnet — Rechnung #N"). Übergreifend wird auf der Athleten-Detailseite ein farbiges Monatsraster („Abgerechnete Monate") angezeigt, das je Monat den Status der abdeckenden Rechnung visualisiert (Erstellt / Versendet / Bezahlt / Entwurf / nicht abgerechnet / nicht aktiv). Klick auf einen Monatschip springt zur passenden Zeile in der darunterliegenden Rechnungstabelle (`#invoice-N`).

### 4.3 PDF-Ausgabe (`FR-P-*`)

- **FR-P-01** — Jede Rechnung ist **einseitig, wenn möglich**. Bei vielen Positionen (mehrmonatige Rechnung mit vielen Zeilen) erfolgt ein **automatischer Seitenumbruch** über die PDF-Engine; Inhalt darf nie abgeschnitten werden. Total- und Bank-Block landen auf der letzten Seite.
...
| 2026-05-31 | FR-I-06 | Eine Rechnung enthält 1–n Positionszeilen (statt zwingend eine). Vorbereitung für mehrmonatige Einzelrechnungen. |
| 2026-05-31 | FR-I-10 | Mehrmonatige Einzelrechnung im Einzel-Flow eingeführt: Zeilen-Builder, eine Rechnungsnummer pro Rechnung, Duplikat- und Inaktiv-Validierung; Monatsbatch dedupliziert gegen mehrmonatige Rechnungen. |
| 2026-05-31 | FR-I-11 | Bereits-abgerechnet-Hinweise im Einzel-Flow (Zusammenfassung + Inline-Badge je Zeile); Monatsraster „Abgerechnete Monate" auf der Athleten-Detailseite mit Sprung zur Rechnung. |
| 2026-05-31 | FR-P-01 | Einseitigkeit ist Ziel, nicht Pflicht: automatischer Seitenumbruch für mehrzeilige (mehrmonatige) Rechnungen erlaubt. |
| 2026-05-31 | FR-I-09 | „Mit selber Nummer neu erstellen" ist im Einzel-Flow nur für einmonatige Rechnungen verfügbar — bei mehrmonatigen Rechnungen muss der Coach manuell löschen. |
| 2026-05-31 | Schema | Store-Schema v3 → v4: `Invoice.Lines []InvoiceLine` + `Invoice.Total Money` eingeführt; v3-Skalare (`month`, `description`, `period_from`, `period_to`, `amount_cents`, `pro_rata`) werden beim Laden einmalig in eine 1-elementige Lines-Liste gefaltet und danach aus dem JSON entfernt. |
